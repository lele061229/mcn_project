...
createApp({
  setup() {
    ...
    const nav = [
      ...
      { key: 'talents', label: '达人库', desc: '每个达人的来源、任务与贡献' },
    ...
    const icons = {
      ...
      talents: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-5 h-5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>',
    ...
    nav.forEach(n => (n.icon = icons[n.key]));
    ...
    const talents = reactive([
      { id: 'T001', name: '小王超会穿', avatar: AV[0], channel: '朋友圈', owner: '李婷', join: '2026-08-30', status: '活跃' },
      { id: 'T002', name: '阿May穿搭日记', avatar: AV[2], channel: '抖音短视频', owner: '王浩', join: '2026-08-29', status: '活跃' },
      { id: 'T003', name: '大琪爱吃穿', avatar: AV[4], channel: '社群推广', owner: '张萌', join: '2026-09-01', status: '活跃' },
      { id: 'T004', name: '小林的日常', avatar: AV[1], channel: 'AI数字人直播', owner: '李婷', join: '2026-09-02', status: '活跃' },
      { id: 'T005', name: '蕾蕾Lena', avatar: AV[5], channel: '达人转介绍', owner: '王浩', join: '2026-09-01', status: '活跃' },
      { id: 'T006', name: '橙子味汽水', avatar: AV[2], channel: '抖音短视频', owner: '王浩', join: '2026-09-03', status: '观望' },
      { id: 'T007', name: '阿呆不呆', avatar: AV[3], channel: '朋友圈', owner: '李婷', join: '2026-09-04', status: '观望' },
      { id: 'T008', name: '桃子peach', avatar: AV[0], channel: '社群推广', owner: '张萌', join: '2026-09-02', status: '观望' },
      { id: 'T009', name: '周周有好货', avatar: AV[4], channel: 'AI数字人直播', owner: '李婷', join: '2026-09-05', status: '活跃' },
      { id: 'T010', name: '大熊的鞋柜', avatar: AV[1], channel: '抖音短视频', owner: '王浩', join: '2026-08-31', status: '活跃' },
      { id: 'T011', name: '小鹿乱撞', avatar: AV[5], channel: '海报图文', owner: '王浩', join: '2026-09-06', status: '观望' },
      { id: 'T012', name: '菲菲女王', avatar: AV[4], channel: '达人转介绍', owner: '张萌', join: '2026-09-04', status: '活跃' },
    ]);
    const bgs = ['from-indigo-400 to-violet-500', 'from-rose-400 to-orange-400', 'from-sky-400 to-cyan-500', 'from-emerald-400 to-teal-500', 'from-fuchsia-400 to-pink-500', 'from-amber-400 to-yellow-500'];
    ...
    // 任务附带达人名称
    tasks.forEach(t => {
      const tl = talents.find(x => x.id === t.talentId);
      t._talentName = tl ? tl.name : '（未知达人）';
    });

    // 达人聚合统计
    const talentCards = computed(() => talents.map((t, i) => {
      const ts = tasks.filter(x => x.talentId === t.id && stageIdx(x.status) >= 6); // 已发布起算佣金
      const commission = ts.reduce((a, b) => a + b.commission, 0);
      const all = tasks.filter(x => x.talentId === t.id);
      return { ...t, _bg: bgs[i % bgs.length], _tasks: all.length, _commission: commission, _mcn: commission * MCN_RATE };
    }));
    ...
    const monthNewTalents = computed(() => talents.filter(t => t.join >= '2026-09-01').length);
    ...
    // 看板 KPI
    const kpis = computed(() => [
      { label: '本月新增达人', value: monthNewTalents.value, unit: '人', good: true, note: '累计达人 ' + talents.length + ' 人' },
    ...
    function confirmImport() {
      ...
      imp.rows.forEach(r => {
        ...
        if (imp.target === 'leads') {
          ...
          talents.push({
            id: 'T' + String(talents.length + 1).padStart(3, '0'), name: obj.name, avatar: AV[count % AV.length],
            channel: obj.channel || '未填写', owner: obj.owner || '未分配', join: obj.join || today, status: obj.status || '观望',
          });
          count++;
        } else {
          const tl = talents.find(t => t.name === obj.talentName);
          tasks.unshift({
            id: obj.id || 'TK' + Date.now(), talentId: tl ? tl.id : 'T001',
    ...
    return {
      page, nav, currentNav, today, toast, trendUp, trendFlat,
