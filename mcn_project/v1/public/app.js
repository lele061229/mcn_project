...
createApp({
  setup() {
    /* ---------- 基础状态 ---------- */
    const _VALID_PAGES = ['dashboard', 'talents', 'talent', 'tasks', 'logs'];
    const _initPage = new URLSearchParams(location.search).get('page');
    const page = ref(_VALID_PAGES.includes(_initPage) ? _initPage : 'dashboard');
    const toast = ref('');
    const users = USERS;
    ...
    (async () => {
      ...
      // 支持从原系统侧边栏 /admin/?page=xxx 直达对应页面并加载数据
      if (page.value === 'talents') await loadTalents();
      else if (page.value === 'tasks') await loadTasks();
      else if (page.value === 'logs') await loadLogs();
